#!/usr/bin/env python3
"""Run X-Plane SIL matrix campaigns for issue #25 (pyEfis visual checks)."""
...
SCENARIOS: List[Dict[str, Any]] = [
    {"id": "25.1", "name": "pyefis_25_1_cdi_tracking", "cycles": 360, "hz": 20, "gust": False},
    {"id": "25.2", "name": "pyefis_25_2_altitude_vs", "cycles": 420, "hz": 20, "gust": False},
    {"id": "25.3", "name": "pyefis_25_3_mode_annunciation", "cycles": 360, "hz": 20, "gust": False},
    {"id": "25.4", "name": "pyefis_25_4_frame_rate", "cycles": 300, "hz": 20, "gust": False},
    {"id": "25.5", "name": "pyefis_25_5_engine_display", "cycles": 480, "hz": 20, "gust": False},
]
...
def _build_manifest(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if not selected:
        raise ValueError("No scenarios selected. Use --tests with values like 25.1 25.2")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run issue #25 pyEfis SIL matrix")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8086)
    parser.add_argument("--repeats", type=int, default=1)
    parser.add_argument("--tests", nargs="*", help="Subset test IDs, e.g. 25.1 25.2")
    ...
    manifest_path = manifest_dir / f"pyefis_matrix_{ts}.json"

    manifest = _build_manifest(args)
    manifest_path.write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    print(f"[pyefis-matrix] manifest: {manifest_path.relative_to(repo_root)}")
    ...
    print("[pyefis-matrix] command:", " ".join(cmd))
